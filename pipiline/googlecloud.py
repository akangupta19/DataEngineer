...
def MLmodel(data):
   ...
   #filename = "gs://akanksha_bucket_1/salary_data3.pkl"
   #with tf.io.gfile.GFile(filename, 'wb') as f:
    #pickle.dump(regressor, f)
     #red = regressor.predict(X_test)

    storage_client = storage.Client()
    bucket = storage_client.get_bucket("akanksha_bucket_1")
    blob = bucket.blob("salary_data3.pkl")
    model_local = "salary_data3.pkl"
    ...
    l = [{
        ...
        'Years': ''.join(e for e in str(X) if e.isalnum()),
        'Salary': ''.join(e for e in str(pred) if e.isalnum()),
